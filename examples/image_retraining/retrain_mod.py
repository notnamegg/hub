...
def create_module_graph(module_spec, is_training=False):
  ...
  tf.logging.info(str(width)+'x'+str(height))
  with tf.Graph().as_default() as graph:
    #resized_input_tensor = tf.placeholder(tf.float32, [None, 299, 299, 3])
    ...
    if is_training:
        ...
        m = hub.Module(module_spec)
    tf.logging.info("get_signature_names"+str(m.get_signature_names()))
    #ifv= m(resized_input_tensor,signature='image_feature_vector',as_dict=True)
    #bottleneck_tensor =ifv['default']
    bottleneck_tensor =m(resized_input_tensor)
    #tf.logging.info("ifv"+str(ifv))
...
def run_bottleneck_on_image(sess, image_data, image_data_tensor,
  ...
  return bottleneck_values, resized_input_values


def run_decode_on_image(sess, image_data, image_data_tensor,
                            decoded_image_tensor, resized_input_tensor,
                            bottleneck_tensor):
  """Runs inference on an image to extract the 'bottleneck' summary layer.
...
def create_bottleneck_file(bottleneck_path, image_lists, label_name, index,
  ...
  try:
    bottleneck_values,resized_input_values = run_bottleneck_on_image(
...
def cache_bottleneck_file(bottleneck_path, image_lists, label_name, index,
                           image_dir, category, sess, jpeg_data_tensor,
                           decoded_image_tensor, resized_input_tensor,
                           bottleneck_tensor, cache_image):
  """Cache a single bottleneck file."""
  if category != "training" or not cache_image:
    return None
  #tf.logging.info('Cacheing bottleneck at ' + bottleneck_path)
  #tf.logging.info(category)
  image_path = get_image_path(image_lists, label_name, index,
                              image_dir, category)
  if not tf.gfile.Exists(image_path):
    tf.logging.fatal('File does not exist %s', image_path)
  image_data = tf.gfile.FastGFile(image_path, 'rb').read()
  try:
    resized_input_values = run_decode_on_image(
        sess, image_data, jpeg_data_tensor, decoded_image_tensor,
        resized_input_tensor, bottleneck_tensor)
  except Exception as e:
    raise RuntimeError('Error during processing file %s (%s)' % (image_path,
...
def get_or_create_bottleneck(sess, image_lists, label_name, index, image_dir,
                             ...
                             decoded_image_tensor, resized_input_tensor,
                             bottleneck_tensor, module_name, cache_image=False):
  ...
  if not os.path.exists(bottleneck_path):
    resized_input_value = create_bottleneck_file(bottleneck_path, image_lists, label_name, index,
                           image_dir, category, sess, jpeg_data_tensor,
                           decoded_image_tensor, resized_input_tensor,
                           bottleneck_tensor)
    if not cache_image:
  ...
  if did_hit_error:
    ...
    # fresh creation
    bottleneck_values = [float(x) for x in bottleneck_string.split(',')]
  return bottleneck_values,resized_input_value
...
def get_random_cached_bottlenecks(sess, image_lists, how_many, category,
  ...
  if how_many >= 0:
    ...
    for unused_i in range(how_many):
      ...
      if resized_input_values is not None:
          resized_input_values = np.concatenate([resized_input_values,resized_input_value],0)
      else:
          resized_input_values = resized_input_value
  else:
    ...
    for label_index, label_name in enumerate(image_lists.keys()):
      for image_index, image_name in enumerate(
        ...
        ground_truths.append(label_index)
        filenames.append(image_name)
  return bottlenecks, ground_truths, filenames, resized_input_values
...
def get_random_distorted_bottlenecks(
  ...
  for unused_i in range(how_many):
    ...
    ground_truths.append(label_index)
  return bottlenecks, ground_truths, distorted_image_data
...
def run_final_eval(train_session, module_spec, class_count, image_lists,
  ...
  Args:
    ...
    bottleneck_tensor: The bottleneck output layer of the CNN graph.
  """
  test_bottlenecks, test_ground_truth, test_filenames,_ = (
...
def add_jpeg_decoding(module_spec):
  ...
  decoded_image = tf.image.decode_jpeg(jpeg_data, channels=input_depth)
  # Convert from full range of uint8 to range [0,1] of float32.
  #decoded_image = tf.image.resize_images(decoded_image,(input_width,input_height))
  decoded_image_as_float = tf.image.convert_image_dtype(decoded_image,
                                                        tf.float32)
  decoded_image_4d = tf.expand_dims(decoded_image_as_float, 0)
...
def main(_):
  ...
  with tf.Session(graph=graph) as sess:
    ...
    for i in range(FLAGS.how_many_training_steps):
      ...
      if do_distort_images:
        (train_bottlenecks,
         train_ground_truth, resized_image_value) = get_random_distorted_bottlenecks(
             sess, image_lists, FLAGS.train_batch_size, 'training',
             FLAGS.image_dir, distorted_jpeg_data_tensor,
             distorted_image_tensor, resized_image_tensor, bottleneck_tensor)
      else:
        (train_bottlenecks,
         train_ground_truth, _, resized_image_value) = get_random_cached_bottlenecks(
             sess, image_lists, FLAGS.train_batch_size, 'training',
      ...
      train_summary, _ = sess.run(
          ...
          feed_dict={bottleneck_input: train_bottlenecks,
                     ...
                     ,resized_image_tensor: resized_image_value
                    })
